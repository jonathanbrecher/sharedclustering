        ...
        private static void ReadMatchFile(Serialized serialized, string matchFile, IProgressData progressData)
        {
            ...
            using (var package = new ExcelPackage(fileStream))
            {
                ...
                { 
                    ...
                    {
                        if (worksheetNumber < numWorksheets && !ws.Name.StartsWith("AutoCluster", StringComparison.OrdinalIgnoreCase))
                        {
                            continue;
                        }
                        ...
                        for (var col = 1; col < 1000; ++col)
                        {
                            ...
                            else if (cellValue.Equals("identifier", StringComparison.OrdinalIgnoreCase))
                            {
                        ...
                        for (var row = 2; row <= maxRow; ++row)
                        {
                            ...
                            if (hyperlinkColumn != 0 && ws.Cells[row, hyperlinkColumn].Hyperlink != null)
                            {
                                try
                                {
                                    // new format
                            ...
                            else if (identifierColumn != 0)
                            {
                                try
                                {
                                    // new format
                                    var url = ws.Cells[row, identifierColumn].Hyperlink.ToString();
                                    var name = ws.Cells[row, identifierColumn].GetValue<string>();
                                    var path = url.Split('/');
                                    resultMatch.MatchTestDisplayName = name;
                                    serialized.TestTakerTestId = path[4];
                                    resultMatch.TestGuid = path[6];
