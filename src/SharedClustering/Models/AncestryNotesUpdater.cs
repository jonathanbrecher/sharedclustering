...
namespace AncestryDnaClustering.Models
{
    internal class AncestryNotesUpdater
    {
        ...
        private async Task<IEnumerable<NotesData>> FilterModifiedNodesAsync(string guid, List<NotesData> notes, HashSet<int> tagIds, Throttle throttle, IProgressData progressData)
        {
        ...
        private async Task SaveUpdatedNotesToFileAsync(List<NotesData> notes, List<Tag> originalTags, IProgressData progressData)
        {
            ...
            using (var p = new ExcelPackage())
            {
                ...
                p.SaveWithRetry(fileName, FileUtils.ShouldRetry, FileUtils.LogException);
