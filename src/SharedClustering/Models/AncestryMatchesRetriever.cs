...
namespace SharedClustering.Models
{
    internal class AncestryMatchesRetriever
    {
        ...
        public async Task<List<Match>> GetMatchesAsync(string guid, int numMatches, HashSet<int> tagIds, bool includeAdditionalInfo, Throttle throttle, IProgressData progressData)
        {
            var retryCount = 0;
            var retryMax = 5;
            while (true)
        ...
        public async Task<MatchCounts> GetMatchCounts(string guid)
        {
            ...
            var highestMatchesTask = GetMatchesPageAsync(guid, new HashSet<int>(), 1, false, throttle, suppressProgress);
            var nextHighestMatchesTask = GetMatchesPageAsync(guid, new HashSet<int>(), 2, false, throttle, suppressProgress);
            var thirdCousinsTask = CountThirdCousinsAsync(guid, throttle, suppressProgress);
            var matchesTask = CountMatchesAsync(guid, throttle, suppressProgress);
        ...
        private Task<int> CountThirdCousinsAsync(string guid, Throttle throttle, IProgressData progressData)
        {
        ...
        private async Task<(int fourthCousins, int totalMatches)> CountMatchesAsync(string guid, Throttle throttle, IProgressData progressData)
        {
            try
            {
        ...
        private async Task<int> CountMatches(string guid, Func<Match, bool> criteria, int minPage, int maxPage, Throttle throttle, IProgressData progressData)
        {
        ...
        public async Task<IEnumerable<Match>> GetMatchesPageAsync(string guid, HashSet<int> tagIds, int pageNumber, bool includeAdditionalInfo, Throttle throttle, IProgressData progressData)
        {
        ...
        public async Task<Match> GetMatchAsync(string guid, string testGuid, HashSet<int> tagIds, Throttle throttle, IProgressData progressData)
        {
            var retryCount = 0;
            var retryMax = 5;
            while (true)
        ...
        public async Task<List<int>> GetMatchesInCommonAsync(string guid, Match match, bool noSharedMatches, double minSharedCentimorgans, Throttle throttle, Dictionary<string, int> matchIndexes, bool throwException, IProgressData progressData)
        {
